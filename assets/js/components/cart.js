...
//Funciones
    ...
    <p class="cart__empty--text">No hay productos en el carrito</p>
      ...
      for (const item of cart){
        ...
        <div class="article__content">
          ...
          <button type="button" class="article__btn remove-from-cart" data-id= "${item.id}"><i class='bx bx-trash'></i></i></button>
        </div>
      </article>
 `
      }
 ...
 function showTotal() {
  let total = 0
  for (const item of cart){
    const productFinded = bd.find( p => p.id === item.id)
 ...
 printCart()
 printProducts()
 window.alert('Se realizo su compra')
}
printCart()
//Eventos
